...
#if 0
	for (p = 0; p < (wc_data->w_pos); p++) {
//		if (tmp_param.debug_log) {
		sprintf(cbuff,"%d,%d,%d,%d,%d,%d\n"
			, (w_data->data[p + half] - 128) * 1000
			, freq_data[0].re[p]
			, freq_data[1].re[p]
			, freq_data[2].re[p]
			, (w_data->data[p + half] - 128)
			, (wc_data->data[p + wc_data->start_pos] - 128)
		);
		if (fiolog) fwrite((void *)cbuff, strlen(cbuff), 1, fiolog);
//		}
	}
#endif
